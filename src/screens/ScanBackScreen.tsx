...
type Props = NativeStackScreenProps<RootStackParamList, 'ScanBack'>;

const ScanBackScreen: React.FC<Props> = ({ navigation }) => {
  const [permission, requestPermission] = useCameraPermissions();
  const cameraRef = useRef<CameraViewRef | null>(null);
  const [checklist, setChecklist] = useState([false, false, false]);
  const [isCapturing, setIsCapturing] = useState(false);
  const { setBackImage } = useCheckin();
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      if (photo?.uri) {
        ...
        navigation.navigate('ScanBackChecked');
      } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bottom}>
        ...
        <PrimaryButton
          label={isCapturing ? 'Capturando...' : 'Escanear parte trasera'}
